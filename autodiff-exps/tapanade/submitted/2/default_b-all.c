...
double sqnorm(int d, double *x);
void preprocess_qs(int d, int k, double *icf, double *sum_qs, double *Qdiags);
void Qtimesx(int d, double *Qdiag, double *ltri, double *x, double *out);
void subtract(int d, double *x, double *y, double *out);
...
/*
  Differentiation of sqnorm in reverse (adjoint) mode:
   ...
   Plus diff mem management of: x:in
*/
void sqnorm_b(int d, double *x, double *xb, double sqnormb) {
    int i;
    double out;
    double outb;
    double sqnorm;
...
*/
void log_wishart_prior_b(int p, int k, double wishart_gamma, double wishart_m,
        double *sum_qs, double *Qdiags, double *Qdiagsb, double *icf, double *
...
/*
  Differentiation of subtract in reverse (adjoint) mode:
   gradient     of useful results: *out *x *y
   with respect to varying inputs: *out *x *y
   Plus diff mem management of: out:in x:in y:in
*/
// out = a - b
void subtract_b(int d, double *x, double *xb, double *y, double *yb, double *
        out, double *outb) {
    int id;
    for (id = d-1; id > -1; --id) {
        xb[id] = xb[id] + outb[id];
...
/*
  Differentiation of gmm_objective in reverse (adjoint) mode:
   ...
   RW status of diff variables: alloc(*Qxcentered):out alloc(*xcentered):out
                alloc(*Qdiags):out alloc(*sum_qs):out alloc(*main_term):out
                *err:in-out *x:out *means:out *icf:out *alphas:out
...
void gmm_objective_b(int d, int k, int n, double *alphas, double *alphasb, 
        ...
        double *xb, double wishart_gamma, double wishart_m, double *err, 
        double *errb) {
    int ik, ix, icf_sz;
    ...
    result1b = *errb;
    log_wishart_prior_b(d, k, wishart_gamma, wishart_m, sum_qs, Qdiags, 
    ...
    *meansb = 0.0;
    for (ix = n-1; ix > -1; --ix) {
        ...
        for (ik = k-1; ik > -1; --ik) {
            ...
            if (branch == 1)
                popreal8(xcentered);
            subtract_b(d, &x[ix*d], &xb[ix*d], &means[ik*d], &meansb[ik*d], 
                       xcentered, xcenteredb);
        }
    }
